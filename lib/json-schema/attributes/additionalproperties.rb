    ...
    class AdditionalPropertiesAttribute < Attribute
      def self.validate(current_schema, data, fragments, processor, validator, options = {})
        ...
        if extra_properties.any? && (addprop == false || (addprop.is_a?(Hash) && !addprop.empty?))
          ...
          validation_error(processor, message: message, fragments: fragments, properties: extra_properties,  schema: current_schema, failed_attribute: self, record_errors: options[:record_errors])
        end
      end
